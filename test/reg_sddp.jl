...
@testset "Regularized Brazilian" begin
    ...
    D1 = [PolyhedralFunction(nx, lower_bound) for t in 1:T]
    # Solve with regularized SDDP 2
    #V2 = deepcopy(V1)
    objective_primal2 = zeros(Float64, 0)
    objective_dual2 = zeros(Float64, 0)
    K = 9
    for k in 5:K
        τ_tmp = Float64(10^(k-1))
        V2 = deepcopy(V1)
        D2 = deepcopy(D1)
        reg_sol2 = Minicut.regularizedsddp2(bhm, x0, optimizer, V2, D2; n_iter=max_iter, verbose=verbose, τ=τ_tmp, lower_bound=lower_bound, n_cycle=n_cycle, n_pruning = n_pruning, allowed_time = allowed_time, n_warmup = n_warmup, valid_statuses = [MOI.OPTIMAL, MOI.LOCALLY_SOLVED])
        objective_primal2 = push!(objective_primal2, reg_sol2.lower_bound)
        objective_dual2 = push!(objective_dual2, reg_sol2.upper_bound)
        println("Regularized SDDP gap.............: $((abs(reg_sol2.upper_bound - reg_sol2.lower_bound) / abs(reg_sol2.upper_bound))*100)% ")    
    end
    # T = 5, allowed_time = 300, max_iter = 2000
    p = plot(5:K, [objective_primal2, objective_dual2] ,title = "Regularized bounds on V₀(x₀) with varying μ", xlabel = "μ = 10^(k-1)", ylabel = "Value", legend = false)
    hline!([objective_sddp])
    savefig(p, "mu_dependance.pdf")


    # Solve with regularized SDDP with Discount Rule for ub
    optimizer = () -> Gurobi.Optimizer(GRB_ENV)
    V2 = deepcopy(V)
    sol_wellington = Minicut.reg_discount(bhm, x0, optimizer, V2; n_iter=max_iter, verbose=verbose, τ=1e8, n_pruning = n_pruning, allowed_time = allowed_time, n_simus = n_simus, n_warmup = n_warmup, valid_statuses = [MOI.OPTIMAL, MOI.LOCALLY_SOLVED])
